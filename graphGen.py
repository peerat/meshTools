...
from sqlite_utils import LOCAL_TZ, connect_sqlite, ensure_traceroutes_ts_epoch, parse_ts_to_epoch as parse_common_ts_to_epoch
...
def parse_ts_to_epoch(value: Any, *, naive_tz: timezone | Any = timezone.utc) -> Optional[float]:
    return parse_common_ts_to_epoch(value, naive_tz=naive_tz)


def parse_db_ts_for_window_epoch(value: Any) -> Optional[float]:
    """Parse DB ts_utc for --datetime filtering as local wall time."""
    if not isinstance(value, str):
        return parse_ts_to_epoch(value, naive_tz=LOCAL_TZ)
    s = value.strip()
    if not s:
        return None
    if s.endswith("Z"):
        ...
        s = s[:-6]
    return parse_ts_to_epoch(s, naive_tz=LOCAL_TZ)
        ...
        for line in lines:
            ...
            if ts_line is not None:
                if ts_line.tzinfo is None:
                    ts_line = ts_line.replace(tzinfo=LOCAL_TZ)
...
def parse_traceroutes_with_stats_from_db(
    ...
    if dt_window:
        w_start, w_end = dt_window
        # --datetime parser returns naive datetime values in local user time.
        # For DB fallback compare against normalized epoch values.
        w_start_epoch = w_start.replace(tzinfo=LOCAL_TZ).timestamp()
        w_end_epoch = w_end.replace(tzinfo=LOCAL_TZ).timestamp()
    ...
    try:
        ...
        if w_start_epoch is not None and w_end_epoch is not None:
            cur.execute(
                """
                SELECT ts_utc, route_raw, ts_epoch
                FROM traceroutes
                ...
                """,
                (int(w_start_epoch) - 86400, int(w_end_epoch) + 86400),
            )
        else:
            cur.execute(
                """
    ...
    for ts_utc, route_raw, ts_epoch_raw in rows:
        line = str(route_raw or "")
        lines_total += 1
        ts_epoch = float(ts_epoch_raw) if isinstance(ts_epoch_raw, (int, float)) else parse_ts_to_epoch(ts_utc, naive_tz=LOCAL_TZ)
        bin_key: Optional[int] = None

        if w_start_epoch is not None and w_end_epoch is not None:
            window_ts_epoch = parse_db_ts_for_window_epoch(ts_utc)
            if window_ts_epoch is None:
                continue
            if window_ts_epoch < w_start_epoch or window_ts_epoch > w_end_epoch:
                continue
            lines_in_window += 1
            selected_any = True
            selected_rows += 1
