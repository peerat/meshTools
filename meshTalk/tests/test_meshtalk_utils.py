...
from meshtalk.compression import compress_text, MODE_BYTE_DICT, MODE_DEFLATE
...
class MeshTalkUtilsTests(unittest.TestCase):
    ...
    def test_format_meta_incoming_uses_received_timestamp(self) -> None:
        ...
        # Incoming UI should show only receive time (avoid "sent at ... received at ...").
        self.assertIn(f"пришло {expected_hhmm}", meta)
    ...
    def test_format_meta_send_blocked_status_ru(self) -> None:
        ru = format_meta_text(
            "ru",
            delivery=5.0,
            attempts=None,
        ...
        self.assertIn("ошибка (радиоинтерфейс занят)", ru)
    ...
    def test_format_meta_incoming_progress_appends_compression_ru(self) -> None:
        ...
        self.assertIn(f"в {started_hhmm} начали прием, прошло 00:15", ru)
        self.assertIn("сжатие ZLIB 37.4%", ru)

    def test_format_meta_incoming_progress_appends_compression_en(self) -> None:
        started_ts = 1700000000.0
        started_hhmm = time.strftime("%H:%M", time.localtime(started_ts))
        en = format_meta_text(
            "en",
        ...
        self.assertIn(f"at {started_hhmm}", en)
        self.assertIn("elapsed 00:15", en)
        self.assertIn("compression ZLIB 37.4%", en)
    ...
    def test_format_meta_incoming_done_phrase_ru(self) -> None:
        ...
        self.assertEqual(
            ru,
            f"пришло {recv_hhmm} за 01:18, попытки 1, хопов 0, части 3/3",
        )
    ...
    def test_parse_compact_meta_new_deflate_mode(self) -> None:
        mc_block = compress_text("привет", mode=MODE_DEFLATE, preserve_case=True)
        compression, legacy_codec, label = parse_compact_meta(1, mc_block)
        self.assertEqual(compression, 1)
        self.assertEqual(legacy_codec, "deflate")
        self.assertEqual(label, "mc_deflate")
    ...
    def test_parse_key_exchange_frame_with_modes(self) -> None:
        ...
        kind, peer_id, pub_out, peer_modes, peer_caps, peer_wire, peer_msg, peer_mc = parsed  # type: ignore[misc]
        ...
        self.assertIsNone(peer_caps)
        self.assertIsNone(peer_wire)
        self.assertIsNone(peer_msg)
        self.assertIsNone(peer_mc)
    ...
    def test_parse_key_exchange_frame_with_protocol_versions(self) -> None:
        pub_raw = b"B" * 32
        pub_b64 = base64.b64encode(pub_raw)
        payload = (
            b"KR2|!aabbccdd|"
            + pub_b64
            + b"|mt_wire=1|mt_msg=1,2|mt_mc=1|mt_caps=aad_type"
        )
        parsed = parse_key_exchange_frame(
            payload=payload,
            key_req_prefix=b"KR1|",
        ...
        self.assertEqual(pub_out, pub_raw)
        self.assertIsNone(peer_modes)
        self.assertEqual(peer_caps, {"aad_type"})
        self.assertEqual(peer_wire, {1})
        self.assertEqual(peer_msg, {1, 2})
        self.assertEqual(peer_mc, {1})
