...
def run_cmd(cmd: List[str], timeout: int) -> Tuple[int, str, str]:
    ...
    try:
        ...
        stdout = ex.stdout if isinstance(getattr(ex, "stdout", None), str) else ex.output if isinstance(getattr(ex, "output", None), str) else ""
        stderr = ex.stderr if isinstance(getattr(ex, "stderr", None), str) else ""
...
# ==============================================================================
def _normalize_nodes_json_payload(parsed: Any) -> Optional[List[Dict[str, Any]]]:
    ...
    for key in ("nodes", "Nodes", "nodesById"):
        ...
        if isinstance(value, dict):
            ...
            for node_id, item in value.items():
                if not (isinstance(node_id, str) and node_id.startswith("!") and isinstance(item, dict)):
                    continue
                normalized = dict(item)
                if _first_not_none(normalized.get("id"), normalized.get("ID"), normalized.get("nodeId")) is None:
                    normalized["id"] = node_id
                rows.append(normalized)
            return rows or None

    for key in ("mesh", "meshInfo", "info"):
        normalized = _normalize_nodes_json_payload(parsed.get(key))
        if normalized:
            return normalized

    rows = []
    for node_id, item in parsed.items():
        if not (isinstance(node_id, str) and node_id.startswith("!") and isinstance(item, dict)):
            continue
        normalized = dict(item)
        if _first_not_none(normalized.get("id"), normalized.get("ID"), normalized.get("nodeId")) is None:
...
def main() -> int:
    ...
    for json_flag in (["--format", "json"], ["--json"], []):
        ...
        if returncode == 124:
            print(
                f"[ERROR] meshtastic --nodes timed out after {args.timeout}s. RU: meshtastic --nodes превысил таймаут {args.timeout}с.",
                file=sys.stderr,
            )
            return 2
        ...
        normalized_json = _normalize_nodes_json_payload(parsed)
        if normalized_json is not None:
            nodes_json = normalized_json
            nodes_stdout, nodes_stderr = stdout, stderr
            break
    ...
    if isinstance(nodes_json, list):
        ...
        for item in nodes_json:
            ...
            node_id = _first_not_none(item.get("id"), item.get("ID"), item.get("nodeId"))
            if not isinstance(node_id, str) or not node_id.startswith("!"):
                continue

            normalized_nodes.append({
                "id": node_id,
                "user": _first_not_none(item.get("user"), item.get("longName"), item.get("User")),
                "aka": _first_not_none(item.get("aka"), item.get("shortName"), item.get("AKA")),
                "hardware": _first_not_none(item.get("hardware"), item.get("hwModel"), item.get("Hardware")),
                "pubkey": _first_not_none(item.get("pubkey"), item.get("publicKey"), item.get("Pubkey")),
                "role": _first_not_none(item.get("role"), item.get("Role")),
                "position": {
                    "lat": safe_float(_first_not_none(item.get("latitude"), item.get("lat"))),
                    "lon": safe_float(_first_not_none(item.get("longitude"), item.get("lon"))),
                    "alt_m": safe_float(_first_not_none(item.get("altitude"), item.get("alt_m"), item.get("alt"))),
                },
                "battery": {
                    "state": item.get("batteryState"),
                    "percent": safe_float(
                        _first_not_none(item.get("batteryLevel"), item.get("batteryPercent"), item.get("battery"))
                    ),
                },
                "channel_util_pct": safe_float(
                    _first_not_none(item.get("channelUtil"), item.get("channel_util"), item.get("channelUtilization"))
                ),
                "tx_air_util_pct": safe_float(
                    _first_not_none(item.get("txAirUtil"), item.get("tx_air_util"), item.get("txAirUtilization"))
                ),
                "snr_db": safe_float(_first_not_none(item.get("snr"), item.get("SNR"))),
                "hops": safe_int(_first_not_none(item.get("hops"), item.get("Hops"))),
                "channel_index": safe_int(_first_not_none(item.get("channel"), item.get("Channel"))),
                "last_heard": _first_not_none(item.get("lastHeard"), item.get("LastHeard")),
                "since": _first_not_none(item.get("since"), item.get("Since")),
                "raw_nodes_row": item,
            })
    else:
    ...
    if info_returncode == 124:
        print(
            f"[ERROR] meshtastic --info timed out after {args.timeout}s. RU: meshtastic --info превысил таймаут {args.timeout}с.",
            file=sys.stderr,
        )
        return 2
