...
## Что умеют инструменты на практике

Ниже несколько типовых сценариев, которые показывают, что можно делать с этим проектом.

1) **Собирать трассы сети** (логгировать traceroute по активным узлам).
```bash
python3 meshLogger.py --port /dev/ttyUSB0 --hours 12
```

2) **Обновлять локальную базу узлов** (с накоплением истории).
```bash
python3 nodeDbUpdater.py --port /dev/ttyUSB0 --db nodeDb.txt
```

3) **Строить графы сети** из накопленных трасс.
```bash
python3 graphGen.py --rankdir LR --min-edge 3
```

4) **Фильтровать граф по временным интервалам** (по timestamp в строках трасс).
```bash
python3 graphGen.py --datetime "2026-01-22 23:33 - 2026-01-22 23:40"
```
